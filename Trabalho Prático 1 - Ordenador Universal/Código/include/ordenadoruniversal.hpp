...
#include "sorts.hpp" //vector, estatisticas, cstdlib e stdio.h inclusos
#include <cmath> //std::fabs
#include <string> //std::string e st::string.c_str()
...
class ordenadoruniversal {
    public:
        ...
        void ordenadorUniversal(vector<T>& vetor, int minTamParticao, estatisticas& estat);
...
// Usando este dado, o método decide qual é o melhor ordenador a ser utilizado, reduzindo o custo ao máximo.
template<typename T>
void ordenadoruniversal<T>::ordenadorUniversal(vector<T>& vetor, int minTamParticao, estatisticas& estat) {
    try {
    ...
    do {
        try {
            ...
            for (int t = minMPS; t <= maxMPS; t += passoMPS) {
                ...
                ordenadorUniversal(vetorAux, t, estat);
    ...
    do {
        try {
            ...
            for (int i = 0; i < custosQuick.tamanho(); i++) {
                vetorResultante.push_back(std::fabs(custosQuick[i] - custosIns[i]));
            }
