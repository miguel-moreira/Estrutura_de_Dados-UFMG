...
//Similar ao partition do QuickSort, deve ser contabilizada em "estat". Responsável por dividir o vetor e ordenar.
template<typename T>
void merge(vector<T>& vetor, int esquerda, int meio, int direita, estatisticas& estat) {
    estat.incChamadas();
...
template<typename T>
void mergeSort(vector<T>& vetor, int esquerda, int direita, estatisticas& estat) {
    estat.incChamadas();
    ...
    int meio = (esquerda + direita) / 2;
    mergeSort(vetor, esquerda, meio, estat);
    mergeSort(vetor, meio + 1, direita, estat);
    merge(vetor, esquerda, meio, direita, estat);
}
