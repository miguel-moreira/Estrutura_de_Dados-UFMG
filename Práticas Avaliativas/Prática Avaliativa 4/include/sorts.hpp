...
template<typename T>
void insertionSort(vector<T>& vetor, int esquerda, int direita) {
    try {
        for (int i = esquerda + 1; i <= direita; ++i) {
            T chave = vetor[i];
            int j = i - 1;
            while (j >= esquerda and chave < vetor[j]) {
                vetor[j+1] = vetor[j];
                j--;
            }
            vetor[j+1] = chave;
...
template<typename T>
void partition(vector<T>& vetor, int esquerda, int direita, int* i, int* j) {
    try {
        *i = esquerda;
        *j = direita;
...
template<typename T>
void quickSort(vector<T>& vetor, int esquerda, int direita, int minTamParticao) {
    try {
        int i, j;
        partition(vetor, esquerda, direita, &i, &j);

        if (esquerda < j and j - esquerda < minTamParticao)
            insertionSort(vetor, esquerda, j);
        else if (j > esquerda)
            quickSort(vetor, esquerda, j, minTamParticao);

        if (direita > i and direita - i < minTamParticao)
            insertionSort(vetor, i, direita);
        else if (direita > i)
            quickSort(vetor, i, direita, minTamParticao);
    } catch (...) {
