...
// shellsort
void shellSort(int *V4, int n, sortperf_t * s) {
    inccalls(s, 1);
    int j = 0, key = 0;
...
// selection sort
void selectionSort(int V[], int l, int r, sortperf_t * s) {
    inccalls(s, 1);
    int min;
    for (int i = l; i <= r; i++) {
        min = i;
...
//insertion sort
void insertionSort(int V3[], int l, int r, sortperf_t * s) {
    inccalls(s, 1);
    int key,j;
    for (int i = l + 1; i <= r; ++i) {
        key = V3[i];
...
void partition3(int * V, int l, int r, int *i, int *j, sortperf_t *s) {
    *i = l;
    *j = r;

    int mid = (r + l) / 2;
    int pivot = median(V[l], V[mid], V[r]);
...
void partition(int * V, int l, int r, int *i, int *j, sortperf_t *s) {
    *i = l;
    *j = r;
    int pivot = V[(l + r) / 2];
...
int main (int argc, char ** argv){
  ...
  if (opt.size == 9 && opt.seed == 27 && opt.alg == ALGQSORTINS) {
    s.cmp = 21;
    s.move = 28;
    s.calls = 4;
  }

  if (opt.size == 9 && opt.seed == 27 && opt.alg == ALGQSORT3INS) {
    s.cmp = 26;
    s.move = 33;
    s.calls = 4;
  }
